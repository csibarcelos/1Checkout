...
type ProductRow = Database['public']['Tables']['products']['Row']; // Adicionado
type AuditLogEntryRow = Database['public']['Tables']['audit_log_entries']['Row']; // Adicionado
  ...
  getAllPlatformProducts: async (accessToken: string | null): Promise<AppProduct[]> => {
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(error.message || 'Falha ao buscar todos os produtos da plataforma.');
      }
      return data ? data.map(fromSupabaseProductRow) : [];
    } catch (genericError: any) {
      console.error('Exception in getAllPlatformProducts:', genericError);
      throw new Error(genericError.message || 'Falha geral ao buscar todos os produtos.');
    }
  },
};
